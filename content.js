window.KYDW = {
  ...
  experience: {
    ...
    practiceGuidance: "实践项目可以下载到自己的电脑上运行；如果还不熟悉代码，建议直接在 Kaggle 上打开对应项目，点击“复制并编辑”保存到自己的账户，再按单元格逐步运行、修改并观察结果。看不懂时，可以向 AI 询问当前单元格在做什么，再根据提示一步步尝试。逐步借助 AI 读懂并完成代码，已经是当前科研实践中的必备能力。即使暂时没有完成，也可以打开实践项目参考答案核对。",
    practiceTaskGuides: {
      "project-01": [
      ...
      "project-02": [
        ...
        { task: "任务 4：补全一次训练更新", fill: "依次完成清梯度、前向计算、BCE/Dice 损失、反向传播和优化器更新。", basis: "根据 bce、dice_score、model 和 opt 的已有定义，以及 history 的记录位置填写。", check: "确认训练损失、验证损失和 Dice 曲线有数值，模型参数在更新。" },
      ...
      "project-03": [
        ...
        { task: "任务 2：补全生成器", fill: "完成 Generator 的线性层、反卷积/上采样模块和输出层，使潜变量变成固定尺寸的单通道胸片。", basis: "根据 LATENT、目标图像尺寸、通道数和最后一层激活函数确定每层输入输出。", check: "输入一批随机噪声，确认生成结果形状和真实胸片一致，数值范围符合归一化设定。" },
