...
- 汤昊天简介唯一口径：`content.js` 的 `team.leaders` 中，`preview` 与 `paragraphs` 均使用“东北大学生物医学工程专业 2023 级，现任 KYDW 2023 级负责人。以第一作者发表 CMBBE、MICCAI2025-CMMCA、Expert Systems With Applications论文。”不得自行增删任职、学校、作者或成果信息。
- 三份宣传文案固定位置：`docs/promotions/公众号公告.md`、`docs/promotions/普通群聊宣传.md`、`docs/promotions/项目专属群公告.md`。它们是独立文案；项目名称、开放状态、日期或入口变化时三份一起检查。
- 项目 02（公开编号 02，站内 `project-03`）的源材料是 `网站内容\体验项目\体验实践项目00-05\体验实践项目02.ipynb`：`Chest X-Ray Images (Pneumonia)` 的 `NORMAL` 胸部 X 射线，使用 `ResidualConvVAE`。不得改写成 MRI，也不得恢复旧 DCGAN、判别器或对抗训练表述。
...
1. 先改唯一来源：网站事实改 `content.js`，实践题目和注释改 `experience/practice/project-*.ipynb`；不要直接改生成的 Kaggle 暂存副本或参考副本。
2. Notebook 改完后运行 `python tools\sync_kaggle_notebook_copies.py`。脚本从题目版生成参考版，并以注释比较阻止题目版与参考版漂移；断言失败不得继续上传。
3. Kaggle 记录必须写明 `owner/slug`、题目/参考副本、来源文件或哈希、状态、核验日期和证据；统一格式为 `YYYY-MM-DD | a02 | practice=... | reference=... | source=... | status=... | checked=... | evidence=...`。`a00/a01/a02` 的题目 slug 分别为 `liuhanyu1007/kydw-try-a00`、`liuhanyu1007/kydw-try-a01`、`liuhanyu1007/kydw-try-a02`，参考副本在 slug 后加 `-reference`。
4. 状态只使用：`本地已生成`（仅脚本成功）、`远端已上传`（远端状态命令确认）、`公开可读`（公开 URL 回读标题/版本）、`内容已核对`（回读并比较题目、任务和注释）或 `未核实`。认证、网络或远端状态不可用时保留日志并写 `未核实`，不得根据本地文件时间推断已同步。
5. 发布前检查 `content.js`、公开 HTML、Notion 公共副本和三份宣传文案：不得出现用户任务原文、AI 内部对话、制作/审核编排、`当前网站工作性编排`、`发布后回填` 等内部措辞。面向学生的正常 AI 使用提示可以保留；内部记录只留在本维护文档或内部归档中。

## 主页地址规则

- 当前唯一公开主页：`https://lhy1007.github.io/KYDW_TRY/`。
- `kydw.is-a.dev` 的注册申请尚未合并，当前访问会跳转到 is-a.dev 的可用域名页面；审核完成、DNS 生效并通过线上回读前，不得把它写入首页、仓库主页元数据、README 或任何公开宣传入口。
- GitHub 仓库的主页元数据也必须保持为上述 GitHub Pages 地址。

## 一、维护目标

网站首先说明 KYDW 是什么，再分别承载团队介绍、项目与活动、资源中心和人员招募。任何新增内容都必须进入明确的合集或项目，不在首页孤立增加专题。
...
专业、行业和细分研究方向归入资源中心的“专业解读”；教学、实践和答案归入项目详情与教学文档库；课程、培训和专题交流归入“项目与活动”；当前与历史招募归入“人员招募”。
...
当前公开状态：Week 1—2 已开放；Week 3—5 仍由数据层和渲染层锁定。体验项目 00—05 可进入对应材料。项目 01—05 的进阶教学、进阶实践、实践项目参考答案、设计报告模板、参考设计报告和随附说明文件当前均不进入公开仓库；相关源文件保存在仓库外的本地内部归档，只有用户明确开放后才恢复发布。

已登记项目按编号对应 Kaggle Notebook 标题 `KYDW_TRY_A00`、`KYDW_TRY_A01` 等；后续项目继续按同一编号和入口规则追加，不改变上层目录。项目详情中的每个材料卡只保留一个“打开”按钮，不在项目详情页额外添加实践项目、参考答案或 Kaggle 跳转块；Kaggle 入口分别放在打开后的实践项目页和实践项目参考答案页。实践项目页把“复制并编辑到自己的账户后运行和修改”作为首选方式，下载到电脑运行是补充；页面中的真实参考输出放在全部实践代码之后。当前 00—05 已接入真实运行的图像和 JSON 结果，未开放项目不生成公开材料入口，新增项目完成真实运行后再按同一映射接入。遇到不理解的代码时，先结合单元格说明、教材和公开资料核对，完成后使用“实践项目参考答案”核对。

当前前三个体验项目的结果基线：项目 00 使用 MNIST 真实 20 轮训练并展示输入/预测对比；项目 01 使用 MRI 真实 20 轮训练，结果图只展示真实阳性 mask，并在对比图中编码 TP/FP/FN；项目 02 使用 NORMAL 胸片 ResidualConvVAE 真实 50 轮训练，展示输入、重建、潜空间采样、训练曲线、最近邻和插值结果。项目 02 不得恢复旧的 DCGAN、判别器或对抗训练表述。学生版 Notebook 和 Kaggle Notebook 必须保留粗体“需要完成”、任务依据、检查方式以及待修改代码前后的整行注释；参考版 Notebook 补齐同一位置的完整实现。
- 体验版只用于完成和理解实践内容，不设置提交报告、撰写报告或报告提交要求；“记录结果”“说明观察”属于页面内的学习动作，可以保留。进阶版的设计报告、方法选择和验证说明只写在进阶材料及体验项目的进阶关系说明中。所有“需要完成的实践任务”板块的简介说明填写位置、确定依据和完成后的检查方式。
...
- 首页团队近期动态直接读取 `team.news` 全部条目；动态卡片保持块内滚动，新增记录应按时间倒序放在数组中，首页和动态详情页始终使用同一份数据。
- Notion 主页与 GitHub 首页同步当前公开动态；Notion“代表性成果”页保留完整动态。
- 论文动态只列用户确认的团队成员；用户未确认的英文作者或老师直接删除。崔涵禹（23级）、苗正洋、伍东辰统一使用中文名，不得在公开动态中恢复未确认的英文写法。
- 期刊条目必须注明中科院分区；用户已明确为一区 TOP 的保持原表述，其他当前展示期刊标为中科院一区。会议条目必须注明 CCF 评级；MICCAI 和 ICASSP 使用“CCF B 类国际顶级”及完整会议名称。
- 论文状态统一使用“发表于”“收录于”等直接表述；会议、期刊和论文题目按现有加粗与斜体规则呈现。
...
- 桌面端采用两列组合：左侧为成员高校和负责人，右侧为代表性成果和团队近期动态。
- 成员高校卡片按“直博、国内学硕/海外研究型硕士、在校本科生”三组纵向展示，默认完整显示，不在成员高校卡片内截断；负责人列表与团队近期动态分别在自己的卡片内滚动，右侧卡片随左侧总高度自然延展。
...
| `programs/sdu-lesson-02.html` | 历史课程中的胸部 X 射线与生成模型实践说明及对应 Kaggle 入口 |
...
| `experience/project-03.html` | 02 胸部 X 射线与生成模型（ResidualConvVAE） | 体验版材料可用；进阶版区域统一锁定 |
...
| 页面 | 描述 | 返回关系 |
| --- | --- | --- |
| `resources/environment.html` | 科研基础环境准备；区分 Kaggle 实践与本地长期环境，说明账户、Notebook、计算资源和常见问题 | 返回体验项目或资源中心 |
...
| 文件 | 使用场景 | 内容要求 |
| --- | --- | --- |
| `docs/promotions/公众号公告.md` | 公众号正式发布 | 说明项目定位、开放时间、当前开放内容、参加方式和联系渠道；随项目目录扩展更新主题列表，不写固定总数量。 |
| `docs/promotions/普通群聊宣传.md` | 普通群聊预告或转发 | 保留项目变动原因、开放时间、体验重点和站内入口，篇幅短，适合直接发送。 |
| `docs/promotions/项目专属群公告.md` | 已进群同学的首次说明 | 先列环境准备，再列站内项目入口、Kaggle 复制运行方式和常见问题反馈要求；不写网络服务商、兑换码、固定 GPU 型号或固定配额。 |
...
| 新内容 | 放置位置 | 排序与页面结构 |
| --- | --- | --- |
| 团队近期动态 | `team.news`、Notion 主页和成果页 | 最新在前；首页和动态详情页均读取全部条目，首页在固定卡片内滚动浏览 |
...
- 合集页展示同一层级的页面卡片。
- 首页“项目与活动”的当前项目区只把“项目环境准备”做成可点击入口；其他项目只作为“项目预览”展示，不在卡片、周次项目预览或其他项目预览中直达项目页。所有项目预览均为不可点击的静态块，统一通过项目卡片的“查看详情”进入项目主页，再从项目主页进入教学、实践和参考答案。
- 项目详情只展示教学项目、实践项目、实践项目参考答案及必要的报告入口；进阶内容未开放时只显示锁定状态。
- 项目材料卡统一只保留“打开”按钮；实践项目和实践项目参考答案进入各自子页面后，顶部按“返回项目详情 → 下载实践 Notebook → 在 Kaggle 中打开”排列。题目版使用 `kagglePractice`（旧数据可回退到 `kaggle`），参考答案使用 `kaggleReference`；不要在项目主页或“实践项目与参考答案”说明块重复放置 Kaggle 按钮。
- 团队介绍页的“代表性成果（成员一作/项目负责人）”三项直接展示，不使用折叠；成果分类和数量以 `team.achievements` 为准，首页概况只保留指标与动态预览。
...
发布宣传文案前，再检查 `docs/promotions/项目专属群公告.md` 中的站内入口和 Kaggle 入口是否为实际可访问链接；不要把占位符当作正式入口发送。
...
- 本地 `_codex_state/current_task_state.md`：恢复长任务时使用的临时状态文件，已由 `.gitignore` 排除，不发布到网站。
