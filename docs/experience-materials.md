...
原材料的 00—05 不是网站显示编号。网站项目编号整体加一，今后新增材料时先确认这张映射表，再修改页面和内容数据。
...
实践 Notebook 目前由两层共用；如果进阶项目获得独立 Notebook，只需在 `content.js` 的 `advancedPractice` 中登记新路径。
